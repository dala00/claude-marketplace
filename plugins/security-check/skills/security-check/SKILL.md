...
総合レポートの末尾に以下の免責事項を必ず表示する：

> ⚠️ このレポートはAIによる自動判定の結果です。誤検出や過剰な判定が含まれる可能性があります。実際の問題の有無はご自身で確認してください。

## 5. レポートの保存

免責事項を含む総合レポートの表示後、Markdownファイルとして保存するかどうかをユーザーに確認する。保存を希望した場合、レポートを `.md` ファイルとしてプロジェクト内に保存する。
